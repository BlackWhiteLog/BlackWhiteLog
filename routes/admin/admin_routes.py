...
# 이종민 import 추가 import routes.owner.owner_menu_db as owner_db
# import Blueprint 도 추가함.
...
# 이종민 추가 3개 ( admin 권한 체크를 세밀하게 안 넣고, 일단 동작부터 붙이는 단계 ) S

@admin_bp.route("/admin/seller-requests", endpoint="admin_seller_requests")
def admin_seller_requests():
    if not session.get("user_id"):
        flash("로그인이 필요합니다.")
        return redirect(url_for("login.login"))

    items = owner_db.fetch_pending_restaurants()
    return render_template("admin/admin_seller_requests.html", items=items)

@admin_bp.route("/admin/seller-requests/<int:restaurant_id>/approve", methods=["POST"], endpoint="approve_seller_request")
def approve_seller_request(restaurant_id):
    if not session.get("user_id"):
        flash("로그인이 필요합니다.")
        return redirect(url_for("login.login"))

    owner_db.approve_restaurant(restaurant_id)
    flash("판매자 신청을 승인했습니다.")
    return redirect(url_for("admin_seller_requests"))

@admin_bp.route("/admin/seller-requests/<int:restaurant_id>/reject", methods=["POST"], endpoint="reject_seller_request")
def reject_seller_request(restaurant_id):
    if not session.get("user_id"):
        flash("로그인이 필요합니다.")
        return redirect(url_for("login.login"))

    owner_db.reject_restaurant(restaurant_id)
    flash("판매자 신청을 거절했습니다.")
    return redirect(url_for("admin_seller_requests"))

# 이종민 추가 3개 ( admin 권한 체크를 세밀하게 안 넣고, 일단 동작부터 붙이는 단계 ) E
